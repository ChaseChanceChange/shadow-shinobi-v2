...
function render(){
  ...
  if(battle.status==='victory'){
    $('#log').className='log victory';
    $('#log').innerHTML='<b style="color:#e1c180">HUNT COMPLETE</b><br>The Wraith breaks apart beneath the shrine ash.<div class="reward"><span>+ Ancient Echo</span><span>+ Relic Trace</span><span>+ Combat Mastery</span></div>';
  }else if(battle.status==='defeat'){
    $('#log').className='log';
    $('#log').textContent='THE CELL HAS FALLEN.';
  }else{
    $('#log').className='log';
    $('#log').innerHTML=active?`<b>${esc(active.name)}</b> has initiative. Choose the kill route.`:'Resolving…';
  }
}
...
function fxEvent(e){
  ...
  if(e.type==='status'){
    const n=document.createElement('div');n.className='float blood';n.textContent='BLEED';n.style.left=(rect.left-br.left+rect.width/2)+'px';n.style.top=(rect.top-br.top+70)+'px';$('#fx').appendChild(n);setTimeout(()=>n.remove(),900);
  }
  if(e.type==='guard'){
    const n=document.createElement('div');n.className='float good';n.textContent='GUARD';n.style.left=(rect.left-br.left+rect.width/2)+'px';n.style.top=(rect.top-br.top+60)+'px';$('#fx').appendChild(n);setTimeout(()=>n.remove(),900);
  }
  if(e.type==='finisher'){
    for(let i=0;i<6;i++){const s=document.createElement('div');s.className='slash';s.style.left=(rect.left-br.left-20)+'px';s.style.top=(rect.top-br.top+8+i*20)+'px';$('#fx').appendChild(s);setTimeout(()=>s.remove(),350)}
  }
}
