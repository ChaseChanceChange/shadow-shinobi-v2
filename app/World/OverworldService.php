...
final class OverworldService
{
    ...
    {
        if (abs($dx) + abs($dy) !== 1) {
            throw new \InvalidArgumentException('Movement must be exactly one tile.');
        }
    ...
    public static function walkable(int $x, int $y): bool
    {
        ...
        $blocked = [
            [4,38,9,4],[18,37,2,9],[27,48,4,6],[34,39,3,7],[41,34,5,3],
            [49,48,4,6],[53,36,4,5],[62,45,3,6],[72,38,4,4],[83,47,5,5],
            [20,8,5,5],[28,21,6,4],[36,7,4,6],[43,17,5,4],[51,5,6,4],
            [60,22,7,5],[68,7,5,5],[84,9,4,6],[75,27,8,3],
        ];
        foreach ($blocked as [$bx,$by,$bw,$bh]) {
    ...
    {
        foreach (self::map()['zones'] as $zone) {
    ...
    private static function decorateState(array $row, ?string $message = null): array
    {
        ...
        $state = [
            ...
            'steps'=>(int)$row['steps'],
            'discovered'=>json_decode((string)$row['discovered_json'], true) ?: [],
    ...
    private static function rollEncounter(array $zone, int $x, int $y, int $steps): ?array
    {
        ...
        $catalog = EncounterCatalog::all();
        $keys = array_keys($catalog);
        $threat = match ((string)$zone['type']) {
            'safe' => 0,
            'ruins' => 4,
            'wild' => 5,
            'district' => 6,
        ...
        return [
            'key'=>$picked,
            'name'=>$catalog[$picked]['name'],
            'threat'=>(int)$catalog[$picked]['threat'],
            'zone'=>(string)$zone['name'],
            'reason'=>'A hostile presence has found your trail.',
        ];
    }
    ...
    {
        if (($state['zone']['key'] ?? '') !== ($old['zone']['key'] ?? '')) {
            return 'Entering ' . (string)$state['zone']['name'] . '.';
        }
